...
class StockSafePositionWizard(models.TransientModel):
    _name = 'stock.sale.position.wizard'
    _description = 'Stock Safe Position Wizard'
    ...
    def _prepare_report_data(self):
        ...
        for i in range(max(len(opening_balances),len(transfers_in),len(transfers_out),len(purchases),len(sales),len(adjustments))):
            if len(production) > i:
    ...
    def generate_report(self):
        ...
        return self.env.ref('manufacturing_reports.action_stock_sale_position_pdf').report_action(
