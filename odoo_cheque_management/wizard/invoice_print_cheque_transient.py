    ...
    def set_amount_lines_in_word(self):
        ...
        if self.amount_in_words:
            if self.cheque_book_id.bank_cheque_id.max_char_in_line1:
                ...
                raw_str = self.amount_in_words.replace("Rupee", "Only") 
            
                    
                # max_char = self.cheque_book_id.max_char_in_line1
                # Bilal Commit By default code start
                # line1 = ""
                # line2 = ""
                # total_word = 0
                ...
                # Bilal Commit By default code end
                
                if len(raw_str.split(' ')) >= self.cheque_book_id.bank_cheque_id.max_char_in_line1:
                    # Calculate the middle index of the string
                    middle_index = len(raw_str.split(' ')) // 2

                    # Find the nearest space to avoid splitting words in the middle
                    space_index = raw_str.rfind(' ', 0, middle_index)

                    # Split the string into two parts
                    self.amount_in_words = raw_str[:space_index]
                    self.amount_in_words_line2  = raw_str[space_index + 1:]
                else:
                    # If the string length is 6 or less, don't split it
                    self.amount_in_words = raw_str
