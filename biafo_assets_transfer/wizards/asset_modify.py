...
class AssetModify(models.TransientModel):
    ...
    asset_name = fields.Char('Asset Name', compute='_compute_asset_transfer_name',readonly=False)
    ...
    fixed_asset_account = fields.Many2one('account.account',related='asset_model_id.account_asset_id', readonly=False)
    dep_asset_account = fields.Many2one('account.account',related='asset_model_id.account_depreciation_id', readonly=False)
    exp_asset_account = fields.Many2one('account.account',related='asset_model_id.account_depreciation_expense_id', readonly=False)
    ...
    def _compute_asset_transfer_name(self):
        self.asset_name = f'{self.asset_id.name} -AT '

    @api.depends('asset_id')
    def _compute_asset_transfer(self):
        posted_value = self.env['account.move'].search([('state', '=', 'posted'), ('asset_id', '=', self.asset_id.id)], limit=1,
                                        order="id desc")
        self.current_value = posted_value.asset_remaining_value
        self.depreciated_value = posted_value.asset_depreciated_value
    ...
    def attach_jv_to_asset(self, jv,account_asset):
        last_dep_debit = jv.line_ids.filtered(lambda line_item: line_item.account_id.id == self.fixed_asset_account.id and
