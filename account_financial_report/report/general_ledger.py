...
class GeneralLedgerReport(models.AbstractModel):
    ...
    def _get_analytic_data(self, account_ids):
        ...
        analytic_accounts = analytic_accounts.filtered(
            lambda x: x.id in account_ids
        )
        analytic_data = {}
    ...
    def _create_general_ledger(
        ...
        cost_center_model = self.env['account.analytic.account']
    
        # Search for all cost centers (can be filtered further if needed)
        cost_centers = cost_center_model.search([])
        
        # Extracting the IDs
        cost_center_ids = cost_centers.ids
        
        general_ledger = self.sort_move_lines_by_analytic_distribution(general_ledger)

        # # return general_ledger
        # results = self.find_matching_records(general_ledger, cost_centers)

        general_ledger = self.add_cost_center_details(general_ledger, cost_centers)
        # return general_ledger
        # general_ledger = self.add_cost_center_info_to_move_lines(general_ledger, cost_centers)
        

        general_ledger = self.calculate_new_balance(general_ledger)
        
        return general_ledger


    # def calculate_new_balance(self,data):
    #     # Copy the input data to avoid modifying the original list
    #     data_copy = deepcopy(data)
        
    #     for record in data_copy:
    #         init_balance = record['init_bal']['balance']
            
    #         for move_line in record['move_lines']:
    #             move_line['new_balance'] = init_balance + move_line['debit'] - move_line['credit']
    ...
    #     for record in data_copy:
    #         # Start with the initial balance from 'init_bal'
    #         current_balance = record['init_bal']['balance']
            
    #         for move_line in record['move_lines']:
    #             # Calculate the new balance for the current move line
    #             move_line['new_balance'] = current_balance + move_line['debit'] - move_line['credit']
                
    #             # Update the current balance for the next move line
    #             current_balance = move_line['new_balance']
    ...
    def calculate_new_balance(self, data):
        ...
        for record in data_copy:
            if not record.get('partners', False):
                # Scenario when 'partners' is False
                # Start with the initial balance from 'init_bal'
                current_balance = record['init_bal']['balance']
                
                for move_line in record.get('move_lines', []):
                    # Calculate the new balance for the current move line
                    move_line['new_balance'] = current_balance + move_line['debit'] - move_line['credit']
                ...
                # Scenario when 'partners' is True
                # Iterate through each grouped partner
                for group in record.get('list_grouped', []):
                    # Start with the initial balance from 'init_bal' of the group
                    current_balance = group['init_bal']['balance']
                    
                    for move_line in group.get('move_lines', []):
                        # Calculate the new balance for the current move line
                        move_line['new_balance'] = current_balance + move_line['debit'] - move_line['credit']
    ...
    def sort_move_lines_by_analytic_distribution(self, data: List[Dict]) -> List[Dict]:
        # Function to get a consistent sorting key for analytic_distribution
        def get_analytic_distribution_key(analytic_distribution: Dict) -> str:
            # Convert the distribution to a sorted tuple of key-value pairs to ensure consistency
            return str(sorted(analytic_distribution.items())) if analytic_distribution else ''

        # Iterate through each item in the list
        for item in data:
            # Sort the move_lines based on the analytic_distribution
            item['move_lines'] = sorted(
                item.get('move_lines', []),
                key=lambda x: (
                    x.get('date', ''),  # primary: date (string comparison works if YYYY-MM-DD format)
                    get_analytic_distribution_key(x.get('analytic_distribution', {})),  # secondary
                    list(x.get('analytic_distribution', {}).values())[0] if x.get('analytic_distribution') else 0.0  # tertiary
                ),
                reverse=False  # Oldest dates first, you can change to True for newest first
            )
        return data

    def add_cost_center_info_to_move_lines(self, records, cost_centers):
            # Create a dictionary for fast lookup of cost centers by their ID
            cost_center_dict = {center['id']: center for center in cost_centers}

            for record in records:
                for move_line in record.get('move_lines', []):
                    # Get the analytic distribution keys
                    analytic_keys = move_line['analytic_distribution'].keys()
                    # Initialize the cost center information
                    cost_center_name = None
                    cost_center_code = None

                    # Find if there's any matching cost center for this move line
                    for key in analytic_keys:
                        key = int(key)  # Convert key to integer for matching
                        if key in cost_center_dict:
                            cost_center_name = cost_center_dict[key]['name']
                            cost_center_code = cost_center_dict[key]['code']
                            # Break after finding the first match
                            break

                    # Add cost center info to the move line if a match was found
                    if cost_center_name and cost_center_code:
                        move_line['cost_center_name'] = cost_center_name
                        move_line['cost_center_code'] = cost_center_code

            return records  



    def add_cost_center_details(self, records, cost_centers):
        cost_center_map = {cc['id']: {'name': cc['name'], 'code': cc['code']} for cc in cost_centers}
        
        for record in records:
            for move_line in record['move_lines']:
                analytic_distribution = move_line.get('analytic_distribution', {})
                
                # Iterate over analytic_distribution keys
                for key in analytic_distribution.keys():
                    # Add cost center details based on the key
                    if int(key) in cost_center_map:
                        move_line['cost_center_name'] = cost_center_map[int(key)]['name']
                        move_line['cost_center_code'] = cost_center_map[int(key)]['code']
                    else:
                        move_line['cost_center_name'] = ''
                        move_line['cost_center_code'] = ''
