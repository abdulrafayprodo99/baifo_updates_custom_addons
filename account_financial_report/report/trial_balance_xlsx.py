    ...
    def _get_report_columns(self, report):
        if not report.show_partner_details:
            res = {
                ...
                1: {"header": _("Account"), "field": "name", "width": 30},
                2: {"header": _("Analytic Accounts"), "field": "analytic_name", "width": 30},
                3: {
    ...
    def _get_report_filters(self, report):
        return [
            ...
            [
                ...
                _("No limit") if not report.limit_hierarchy_level else _("Level %s") % (report.show_hierarchy_level),
            ],
        ]
    ...
    def _generate_report_content(self, workbook, report, data, report_data):
        ...
        if not show_partner_details:
            ...
            for balance in trial_balance:
                # Display parent account line with blank analytic account
                parent_line = {
                    "code": balance["code"],
                    "name": balance["name"],
                    "analytic_name": "",
                ...
                self.write_line_from_dict(parent_line, report_data)
                
                # Display analytic account lines under the parent account
                if balance.get("analytic_accounts"):
                    for analytic in balance["analytic_accounts"]:
                        analytic_line = {
                            "code": analytic["code"],
                            "name": "",
                            "analytic_name": analytic["name"],
                            "initial_balance": analytic["initial_balance"],
                            "debit": analytic["debit"],
                            "credit": analytic["credit"],
                            "balance": analytic["balance"],
                            "ending_balance": analytic["ending_balance"],
                        }
                        self.write_line_from_dict(analytic_line, report_data)
        else:
