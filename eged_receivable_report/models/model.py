...
class AgedPartnerBalanceCustomHandler(models.AbstractModel):
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        rslt = super(AgedPartnerBalanceCustomHandler, self)._aged_partner_report_custom_engine_common(options, internal_type, current_groupby, next_groupby, offset=0, limit=None)
        ...
        query = f"""
            ...
            SELECT
                ...
                ARRAY_AGG(DISTINCT COALESCE(DATE '{date_to}' - move.invoice_date,DATE '{date_to}' - move.invoice_date)) AS exact_days,
                ARRAY_AGG(DISTINCT payment_term_line.days) AS payment_term_days,
                ARRAY_AGG(DISTINCT COALESCE((DATE '{date_to}' - move.invoice_date) - payment_term_line.days, (DATE '{date_to}' - move.invoice_date) - payment_term_line.days)) AS over_due,
