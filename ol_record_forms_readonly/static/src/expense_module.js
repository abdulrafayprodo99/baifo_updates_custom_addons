...
class ReadonlyFormController extends FormController {
    ...
    setup(){
        ...
        useEffect(()=>{  
            ...
            this.disableForm()
        }, ()=>[this.model.root.data.expense_module_readonly_check])
    ...
    disableForm(){
        ...
        const check = this.model.root.data.expense_module_readonly_check == true
    ...
    async beforeLeave() {
        if (this.model.root.data.expense_module_readonly_check == true) return
        super.beforeLeave()
    }
...
registry.category("views").add("expense_module_form_disable_readonly", readonlyFormView)