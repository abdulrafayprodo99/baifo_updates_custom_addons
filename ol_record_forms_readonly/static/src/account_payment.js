...
class ReadonlyFormController extends FormController {
    ...
    setup(){
        ...
        useEffect(()=>{  
            ...
            this.disableForm()
        }, ()=>[this.model.root.data.vendor_payment_readonly_check])
    ...
    disableForm(){
        ...
        const check = this.model.root.data.vendor_payment_readonly_check == true
    ...
    async beforeLeave() {
        if (this.model.root.data.vendor_payment_readonly_check == true) return
        super.beforeLeave()
    }
...
registry.category("views").add("vendor_payment_form_disable_readonly", readonlyFormView)