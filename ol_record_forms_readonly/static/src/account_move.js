...
class ReadonlyFormController extends FormController {
    ...
    setup(){
        ...
        useEffect(()=>{  
            ...
            this.disableForm()
        }, ()=>[this.model.root.data.vendor_bills_readonly_check])
    ...
    disableForm(){
        ...
        const check = this.model.root.data.vendor_bills_readonly_check == true
    ...
    async beforeLeave() {
        if (this.model.root.data.vendor_bills_readonly_check == true) return
        super.beforeLeave()
    }
...
registry.category("views").add("vendor_bills_form_disable_readonly", readonlyFormView)