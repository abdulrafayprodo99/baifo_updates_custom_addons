...
# #     inverse_currency_rate = fields.Float('Rate', digits=(12, 6))
# #     manual_currency_rate= fields.Float('Inverse Rate', compute='_compute_inverse_rate', digits=(12, 6))
    
# #     @api.depends('manual_currency_rate')
# #     def _compute_inverse_rate(self):
# #         for record in self:
# #             record.manual_currency_rate = 1 / record.inverse_currency_rate if record.inverse_currency_rate else 0
...
# #     # Overriding the getter of manual_currency_rate field
# #     @api.depends('manual_currency_rate', 'manual_currency_rate_active')
# #     def _get_manual_currency_rate(self):
# #         for record in self:
# #             if record.manual_currency_rate_active and record.manual_currency_rate != 0:
# #                 record.manual_currency_rate = 1 / record.manual_currency_rate
# #                 # raise UserError(record.manual_currency_rate)

# #             else:
# #                 record.manual_currency_rate = record.manual_currency_rate
# #                 # raise UserError(record.manual_currency_rate)

    
# #     def _compute_payments_widget_to_reconcile_info(self):
# #         # raise UserError("Hello  _compute_payments_widget_to_reconcile_info!!")
# #         for move in self:
...
#     do_ref = fields.Char(string='DO Reference')
...
#             if rec.use_manual_rate and rec.manual_currency_rate:  # Check if manual rate is enabled and set
#                 rate_to_use = rec.manual_currency_rate
#             else:
#                 rate_to_use = rec.currency_rate
...
#                         if rate:
#                             rate.write({
#                                 'inverse_company_rate': rate_to_use,
#                                 'currency_id': rec.currency_id.id,
#                             })
#                             # raise UserError("Hello!!")
#                         else:
...
#         def get_odoo_rate(vals):
#             try:
#                 debit_move = debit_vals.get('record').move_id if debit_vals.get('record').move_id else None
#                 rate = debit_move.expense_id.conversion_rate
#                 return 1/rate 
#             except Exception as e:
#                 pass   
#             # raise UserError(vals.get('manual_currency_rate'))
#             if vals.get('manual_currency_rate'):
#                 exchange_rate = vals.get('manual_currency_rate')
#                 # raise UserError(exchange_rate)
#                 return exchange_rate
#             if vals.get('record') and vals['record'].move_id.is_invoice(include_receipts=True):
#                 exchange_rate_date = vals['record'].move_id.invoice_date
#             else:
#                 exchange_rate_date = vals['date']
#             res = recon_currency._get_conversion_rate(company_currency, recon_currency, vals['company'], exchange_rate_date)
#             return res
...
#         if debit_vals.get('record') and debit_vals['record'].move_id.use_manual_rate and debit_vals['record'].move_id.manual_currency_rate:
#             debit_vals['manual_currency_rate'] = debit_vals['record'].move_id.manual_currency_rate
#         if credit_vals.get('record') and credit_vals['record'].move_id.use_manual_rate and credit_vals['record'].move_id.manual_currency_rate:
#             credit_vals['manual_currency_rate'] = credit_vals['record'].move_id.manual_currency_rate
